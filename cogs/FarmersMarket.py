                ...
                with Session() as session:
                    ...
                    if returned_wallet and returned_wallet.balance >= random_price:
                        ...
                        await interaction.followup.send(
                            f"You bought 1 tomato for {random_price}! Total tomatoes: {result_wallet.number_of_tomatoes}.",
                            ephemeral=True
                        )
                    else:
        ...
        async def button2_callback(interaction: discord.Interaction):
            ...
            try:
                with Session() as session:
                    ...
                    if returned_wallet and returned_wallet.number_of_tomatos >= 1:
                        print(f'[{__name__}]: [returned_wallet]: True')
                        ...
                        await interaction.followup.send(
                            f"You planted a tomato for ${random_price}! Wallet Total: {result_wallet.balance}.",
                            ephemeral=True
                        )
                    else:
                        print(f'[{__name__}]: [returned_wallet]: False')
